...
async function getData() {
  const { data, count } = await supabaseAdmin
    ...
    .select('id, codi, descr, type, unit, cost', { count: 'exact' })
    .order('descr')
    .limit(500)
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">{count.toLocaleString('es-ES')} herramientas importadas</p>
      </div>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 font-semibold text-slate-700">{formatCurrency(row.cost)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
